...
void rtcc_read(rtccTimeDate *tm) {
...
void rtcc_write(rtccTimeDate *tm) {
    RTCCLock(1);
    RTCCFG |= 0x03; // set RTCPTR bits to 11
...
BYTE rtcc_get_sec(void) {
    RTCCFG |= 0x00; // set RTCPTR bits to 00
    return mBcd2Dec(RTCVALL); //RTCPTR = 00
}

void rtcc_writestr(rtccTimeDate *tm, char* date_buff, char* time_buff) {
    rtcc_read(tm);
...
UINT32 rtcc_2uint32(rtccTimeDate *tm) {
